<template>
    <div class="orderDetail" id="orderContent"  @mousemove="showPro" v-loading="loading">
        ...
        <div v-if="btnShow"  :modal='false' slot="footer" class="dialog-footer">
        ...
        <div class="tabsMsg">
            <el-tabs v-model="activeName2" type="card" @tab-click="handleClick">
                ...
                <el-tab-pane label="报价信息" name="second" class="offer">
                    ...
                    <h2 style="margin-top:10px;">最新报价信息<span>(报价时间：{{getdata.insTime|times}})</span></h2>
                    <table class="table">
                            <tr>
                                <td><p>上年交强出险: </p></td>
                                <td>-----</td>
                        ...
                        <el-row type="flex">
                            <el-col :span="4" class="inLeft">
                                <div v-for="(item,index) of company" :class="{'orderActive': index == active}" @click="selectOrder(index)" class="offerMsg" :key="index">
                                    ...
                                    <div>
                                        <p class="insurance">保险<span>({{order.policyStatus|state}})</span></p>
                                ...
                                <div class="inRight">
                                    ...
                                    <table border="1"  cellspacing="0" v-if="order.orderStatus!=''">
                                        ...
                                        <tbody>
                                            ...
                                            <tr v-if="getdata.buJiMianSanZheBaoE!=0">
                                                <td>第三者责任不计免赔险</td>
                                                <td>投保</td>
                                                <td>{{getdata.buJiMianSanZheBaoE}}</td>
                                            </tr>
                                            ...
                                            <tr v-if="getdata.forceTotal!=0">
                                                <td>交强险保费合计</td>
                                                <td colspan="2">{{getdata.forceTotal}}</td>
                                            </tr>
                                            <tr v-if="getdata.bizTotal!=0">
                                                <td>商业险保费合计</td>
                                                <td colspan="2">{{getdata.bizTotal}}</td>
                                            </tr>
                                            <tr v-if="getdata.taxTotal!=0">
                                                <td>车船税合计</td>
                                                <td colspan="2">{{getdata.taxTotal}}</td>
                                            </tr>
                                            <tr>
                                                <td>保费总金额</td>
                                                <td colspan="2">{{getdata.zongjine}}</td>
                                            </tr>
                                        </tbody>
                                    </table>
                                </div>
        ...
        <transition name="el-fade-in">
            <OfferAuto @reloadTwo='reloadTwo' @newOffer='newOffer' v-if="showOfferAuto" @close='close'  :data='data' :khid="khid" :orderId="orderId" :order='order' :licenseNo='order.licenseNo'/>
        </transition>
    </div>
</template>
    ...
    methods:{
        ...
        queryOrderRecords(){
            ...
            this.$axios({
                ...
                this.progressTable.forEach(item=>{
                    // item.insTime=commod(item.insTime)
                    // item.nextContactTime=commod(item.nextContactTime)
                    if(item.proStatu){
                        if(item.proStatu==0){
        ...
        showPro(event){
            if(document.body.clientWidth-event.pageX<=300){
