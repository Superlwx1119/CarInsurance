...
export var UMO = {
	 ...
	 * @param w
	 */
	start : function(apiurl, bizurl, evthandler, eid, epwd, aid, apwd, adn, cb,
			w) {
		if (UMO._running == true) {
			if ((cb != null) && (cb != undefined)) {
				cb("/comm/start", {
					"errno" : UMO.ERR_ALREADYSTART,
					"errmsg" : UMO.MSG_ALREADYSTART
				});
			}
			return;
		}
	...
	getevent : function(evttype, cb, w) {
		...
		var args = "dom=" + UMO._dom + "&token=" + UMO._token + "&evttype="
				+ evttype;
	...
	subscribe : function(evttypes, cb, w) {
		...
		var args = "dom=" + UMO._dom + "&token=" + UMO._token + "&evttypes="
				+ evttypes;
	 ...
	 * @param w
	 */
	speeddial : function(teleno, dispno, playfile, oper, param, gid, recflag,
			uud, backurl, cb, w) {
		var cmd = "/tele/call/speeddial";
		var args = "dom=" + UMO._dom + "&token=" + UMO._token + "&teleno="
	 ...
	 * @param w
	 */
	speedplay : function(callid, playfile, dtmfcnt, termchars, loop, async, cb,
			w) {
		var cmd = "/tele/call/speedplay";
		var args = "dom=" + UMO._dom + "&token=" + UMO._token + "&callid="
	 ...
	 * @param w
	 */
	speedrecord : function(callid, filename, async, maxtime, termchar, append,
			cb, w) {
	...
	speedconnect : function(callid1, callid2, action, cb, w) {
		...
		var args = "dom=" + UMO._dom + "&token=" + UMO._token + "&callid1="
	...
	confnew : function(password, cb, w) {
		...
		var args = "dom=" + UMO._dom + "&token=" + UMO._token + "&password="
				+ password;
	...
	confdelete : function(confid, password, cb, w) {
		...
		var args = "dom=" + UMO._dom + "&token=" + UMO._token + "&confid="
	...
	faxdelete : function(faxfile, cb, w) {
		...
		var args = "dom=" + UMO._dom + "&token=" + UMO._token + "&faxfile="
				+ faxfile;
	...
	getrecordfile : function(cdrsrc, cb, w) {
		...
		var args = "dom=" + UMO._dom + "&token=" + UMO._token + "&cdrsrc="
				+ cdrsrc;
	...
	g729decode : function(srcfile, destfile, cb, w) {
		...
		var args = "dom=" + UMO._dom + "&token=" + UMO._token + "&srcfile="
	...
	text2wave : function(text, destfile, option, cb, w) {
		...
		var args = "dom=" + UMO._dom + "&token=" + UMO._token + "&text=" + text
	...
	play : function(filename, tts, async, loop, cb, w) {
		...
		var args = "dom=" + UMO._dom + "&token=" + UMO._token + "&filename="
	 ...
	 * @param w
	 */
	getdtmf : function(filename, tts, loop, maxdigits, termchars, timeout, cb,
			w) {
	...
	stopop : function(taskid, type, cb, w) {
		...
		var args = "dom=" + UMO._dom + "&token=" + UMO._token + "&taskid="
				+ taskid + "&type=" + type;
	...
	setuud : function(uud, cb, w) {
		...
		var args = "dom=" + UMO._dom + "&token=" + UMO._token + "&uud=" + uud;
	...
	sendtextmessage : function(destaid, chatmode, text, cb, w) {
		...
		var args = "dom=" + UMO._dom + "&token=" + UMO._token + "&destaid="
	...
	transferex : function(destno, uud, failaction, failacd, cb, w) {
		...
		var args = "dom=" + UMO._dom + "&token=" + UMO._token + "&destno="
	...
	sendmessagetoide : function(text, cb, w) {
		...
		var args = "dom=" + UMO._dom + "&token=" + UMO._token + "&text="
				+ encodeURIComponent(text);
	...
	datanew : function(data, flds, keyfld, indices, cb, w) {
		...
		var args = "dom=" + UMO._dom + "&token=" + UMO._token + "&data=" + data
	...
	backupnew : function(appname, cb, w) {
		...
		var args = "dom=" + UMO._dom + "&token=" + UMO._token + "&appname="
				+ appname;
	...
	procnew : function(proc, parm, code, cb, w) {
		...
		var args = "dom=" + UMO._dom + "&token=" + UMO._token + "&proc=" + proc
	...
	procexec : function(realm, proc, parms, cb, w) {
		...
		var args = "dom=" + UMO._dom + "&token=" + UMO._token + "&realm="
	...
	fileimport : function(file, data, flds, types, opt, badfile, cb, w) {
		...
		var args = "dom=" + UMO._dom + "&token=" + UMO._token + "&file=" + file
	...
	filedownload : function(scope, object, path, file, cb, w) {
		...
		var args = "dom=" + UMO._dom + "&token=" + UMO._token + "&scope="
	...
	entdeptlist : function(parentid, cb, w) {
		...
		var args = "dom=" + UMO._dom + "&token=" + UMO._token + "&parentid="
				+ parentid;
	...
	entdeptinfo : function(deptid, cb, w) {
		...
		var args = "dom=" + UMO._dom + "&token=" + UMO._token + "&deptid="
				+ deptid;
	...
	entuserinfo : function(userid, cb, w) {
		...
		var args = "dom=" + UMO._dom + "&token=" + UMO._token + "&userid="
				+ userid;
	...
	_tokenvalid : function(cmd, cb) {
		if (UMO._token == "") {
			if (cb != null) {
				var result = {
					errno : UMO.ERR_BADTOKEN,
					errmsg : UMO.MSG_NOTLOGIN
				};
				cb(cmd, result);
			}
			return false;
	...
	_ajaxcall : function(cmd, args, cb, w) {
		...
		$.ajax({
			...
			error : function(xhr, ajaxOptions) {
				if (cb != null) {
					var result = {
						errno : UMO.ERR_AJAXERR,
						errmsg : UMO.MSG_AJAXERR
					};
					cb(cmd, result);
				}
			},
	...
	onData : function(event) {
		...
		case "opercallback":
			if (UMO._evthandler.onOperCallback != null) {
				...
				UMO._evthandler.onOperCallback(flowid, callid, cdrid,
		...
		case "speedcallback":
			if (UMO._evthandler.onSpeedCallback != null) {
				...
				UMO._evthandler.onSpeedCallback(flowid, callid, cdrid,
